...
class unetConv2(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm):
        ...
        if is_batchnorm:
            self.conv1 = nn.Sequential(
                nn.Conv2d(in_size, out_size, 3, 2, 1), nn.BatchNorm2d(out_size), nn.ReLU()
            )
    ...
    def forward(self, inputs):
        ...
        # outputs=self.drp1(outputs)
        return outputs

class unetfirstConv2(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm):
        super(unetfirstConv2, self).__init__()
        self.drp1 = nn.Dropout(p=0.2)
        if is_batchnorm:
            self.conv1 = nn.Sequential(
...
class unetUp(nn.Module):
    def __init__(self, in_size, out_size, is_deconv):
        ...
        self.drp=nn.Dropout(p=0.25)
        self.conv = unetConv2(in_size, out_size, False)
        self.shortcut = nn.Sequential(nn.Conv2d(in_size, out_size, 1, 1, 0),nn.BatchNorm2d(out_size))
        if is_deconv:
    ...
    def forward(self, inputs1, inputs2):
        # print("initial inputs1,inputs2: ",inputs1.shape,inputs2.shape)
        ...
        # print("outputs1 after padding: ",outputs1.shape)
        # print("after concatnating: ",torch.cat([outputs1, outputs2], 1).shape)
        output3=torch.cat([outputs1, outputs2], 1)
        initial=self.shortcut(output3)
        # print("initial.shape ",initial.shape)
        output3=self.conv(output3)
        # print("output3.shape ",output3.shape)
        # print("drop.shape ",self.drp(output3).shape)
        # return self.conv(torch.cat([outputs1, outputs2], 1))
        # return self.conv(output3)
        # output3=self.drp(output3)
        output3=output3+initial
        # print("after addition",output3.shape)
        return output3
