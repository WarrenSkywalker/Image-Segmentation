...
class PolynomialLR(_LRScheduler):
    def __init__(self, optimizer, max_iter, decay_iter=30, gamma=0.9, last_epoch=-1):
    ...
    def get_lr(self):
        ...
        if self.last_epoch == 0 or self.last_epoch % self.decay_iter != 0 or self.last_epoch >= self.max_iter:
            ...
            print("self.last_epoch",self.last_epoch)
            # print(base_lr * factor for base_lr in self.base_lrs)
            for base_lr in self.base_lrs:
                print("current lr: ",base_lr*factor)
                lr_update=base_lr * factor
            self.base_lrs=[base_lr*factor for base_lr in self.base_lrs]
            return [lr_update]
